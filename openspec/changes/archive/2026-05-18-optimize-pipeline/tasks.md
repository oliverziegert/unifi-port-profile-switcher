...
- [x] 8.1 Staged-rollout collapsed: the maintainer pushed everything as a single WIP commit (`e1c6769`) rather than four sequential draft PRs. All three workflow runs on that commit reported green (Builder 2m8s, Go 44s, Lint 27s).
- [x] 8.2 Go CI workflow ran green on the WIP push (44s, all three parallel jobs green, coverage profile uploaded as `coverage` artifact). Nightly cron at `0 3 * * *` UTC will fire on the next day after the push; not blocking and verifiable from the GitHub Actions UI as it accumulates.
- [x] 8.3 Verified via `crane config`:
    1. Docs-only skip not directly exercised in main history (the WIP commit included Go-source changes), but the composite action's filter logic was unit-tested locally across nine scenarios including docs-only.
    2. The WIP commit included Go-source changes (`cmd/root.go`, `internal/unifi/client.go`, etc.); the Builder workflow ran for every architecture (amd64 + aarch64) and republished both per-arch images plus the multi-arch manifest.
    3. `crane config ghcr.io/oliverziegert/unifi-port-profile-switcher:0.0.4` returns the expected labels on both arches: `io.hass.type=addon`, `org.opencontainers.image.revision=e1c6769b3f9ba553cfbe838751f819735c46e6e2` (matches the WIP commit SHA), `org.opencontainers.image.version=0.0.4`, `org.opencontainers.image.created=2026-05-18 20:21:46+00:00` (amd64) / `20:21:49+00:00` (aarch64), plus `io.hass.name`, `io.hass.description`, `io.hass.url`, and the OCI title/description/source/licenses set.
- [x] 8.4 Release workflow exercised end-to-end. `v0.0.0-rc.0` tag triggered a 46s green run (prerelease=true via the `-` in the tag name); `v0.0.4` tag triggered a 42s green run and produced a non-prerelease GitHub Release containing exactly the four binaries plus `SHA256SUMS`. `sha256sum -c SHA256SUMS` reports `OK` for all four. Running `unifi-port-profile-switcher_0.0.4_darwin_arm64 --version` prints `0.0.4`, confirming the `-ldflags -X main.version=` injection works end-to-end.
- [x] 8.5 No residual inline `jq`/bash addon-filter duplication in any workflow file (grep across `.github/` for `Drop directories that match config`, `filtered_apps=(`, or `jq.*Dockerfile` returns zero hits). No dead workflow files / steps observed.
- [x] 8.6 Run `/opsx:archive optimize-pipeline` once 8.4 lands.
